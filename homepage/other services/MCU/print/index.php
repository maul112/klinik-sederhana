...
$title = $_GET["title"];
$temp = mysqli_query($conn, "SELECT * FROM mcu WHERE username = '$username' AND title = '$title' AND status = 'upcoming'");
$hasil = mysqli_fetch_all($temp);
$nomor = count($hasil);
$temp1 = mysqli_query($conn, "SELECT * FROM mcu WHERE username = '$username' AND title = '$title' AND status = 'upcoming'");
